...
def student():
    ...
    addreshlabel = Label(addroot,text='Enter Address : ',bg='gold2',font=('time',20,'bold'),relief=GROOVE,borderwidth=3,width=12,anchor='w')
    addreshlabel.place(x=10,y=250)
...
def addstudent():
    ...
    def submitadd():
        ...
        try:
            ...
            con.commit()
            res=messagebox.askyesnocancel('Notification','id {} Name {} added successfuly.. and want to clean the form'.format(id,name),parent=addroot)
            if(res==True):
                idval.set('')
                nameval.set('')
                mobileval.set('')
...
def searchstudent():
    ...
    searchroot.grab_set()
    searchroot.geometry('470x570+220+200')
    ...
    addreshlabel = Label(searchroot,text='Enter Address : ',bg='gold2',font=('time',20,'bold'),relief=GROOVE,borderwidth=3,width=12,anchor='w')
    addreshlabel.place(x=10,y=250)
...
def updatestudent():
    def update():
        ...
        con.commit()
        messagebox.showinfo('Notifications','Id {} modified successfullly..'.format(id),parent=updateroot)
        strr = 'select * from studentdata2'
        mycursor.execute(strr)
        datas = mycursor.fetchall()
        studenttable.delete(*studenttable.get_children())
    ...
    addreshlabel = Label(updateroot,text='Enter Address : ',bg='gold2',font=('time',20,'bold'),relief=GROOVE,borderwidth=3,width=12,anchor='w')
    addreshlabel.place(x=10,y=250)
...
def exportstudent():
    ...
    dd = ['id','Nmae','Moile','Email','Address','Gender','D.O.B','Added Date','Added Time']
    df=pandas.DataFrame(list(zip(id,name,mobile,email,address,gender,dob,addeddate,addedtime)),columns=dd)
    paths = r'{}.csv'.format(ff)
    df.to_csv(paths,index=False)
    messagebox.showeinfo('Notification','Student data is saved {}'.format(paths))
...
def Connectdb():
    def submitdb():
        ...
        try:
            con=pymysql.connect(host="localhost",user="root",password="@#Ashish!")
            mycursor=con.cursor()
        except:
        ...
        try:
            ...
            mycursor.execute(strr)
            strr = 'alter table studentdata2 modify column id primary key'
            mycursor.execute(strr)
            messagebox.showinfo('Notification ','Database created and Now your are  connected Database......',parent = dbroot)
        except:
            strr = 'use studentmanagementsystem2'
            mycursor.execute(strr)
            messagebox.showinfo('Notification ','Now your are  connected to Database......',parent = dbroot)
        dbroot.destroy()
...
###################################################################### (Intro Slider)
import random
color = ['red','green','blue','yellow','red2','gold2']
def IntroLabelColorTick():
    fg=random.choice(color)
    SliderLable.config(fg=fg)
    SliderLable.after(20,IntroLabelColorTick)
#############################################################################
def IntroLabelTick():
    global count,text
    if(count>=len(ss)):
...
ShowDataFrame.place(x=550,y=80,width=620,height=5)
...
clock.place(x=0,y=0)
tick()
###################################################################################################Connet Databse botton
connectbutton = Button(root,text="Connect to Database",width=23,font=('chiller', 20, 'italic bold'),relief=RIDGE,borderwidth=4,bd=6,bg='green2',
